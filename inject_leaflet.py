...
# Update UI Map
old_ui = """<div x-data="coordinateManager({{ $payload['bag-1']['coordinates_raw'] ?? '[]' }}, $wire)" class="border border-slate-200 rounded-xl bg-slate-50 p-5 shadow-sm">
                                <div class="flex justify-between items-center mb-4">
                                    <div>
                                        ...
                                        <p class="text-xs text-slate-500 mt-1">Isi format Decimal Degree (DD) atau Derajat Menit Detik (DMS). Keduanya akan otomatis tersinkronisasi.</p>
                                    </div>
...
new_ui = """<div x-data="coordinateManager({{ $payload['bag-1']['coordinates_raw'] ?? '[]' }}, {{ $payload['bag-1']['shape_type'] ?? "'polygon'" }}, $wire)" class="border border-slate-200 rounded-xl bg-slate-50 p-5 shadow-sm">
                                <div class="flex justify-between items-center mb-4">
                                    <div>
                                        <h4 class="text-[14px] font-bold text-slate-700 uppercase tracking-wider flex items-center">
                                            <i class="fa-solid fa-map-location-dot mr-2 text-blue-500"></i> Koordinat Lokasi
                                ...
                                <div class="mb-5 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                                    ...
                                    <div id="coordinate-map" wire:ignore class="w-full h-[400px] rounded-lg border border-slate-300 z-0 relative"></div>
                                </div>
...
# Update Script
script_regex = re.compile(r'<script>\s*function coordinateManager\(initialRaw, wire\).*?</script>', re.DOTALL)
new_script = """<script>
...
text = script_regex.sub(new_script, text)
